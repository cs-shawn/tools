...
class TelnetConn(asyncore.dispatcher):
    ...
    def run_script(self, cmd_str):
        ...
        if encoding and not isinstance(cmd_str, str):
    ...
    def writable(self):
        return self.buffer and len(self.buffer) > 0
...
class TelnetServer(asyncore.dispatcher):
    ...
    def update_whitelist(self, whitelist):
        if not whitelist or '__iter__' not in whitelist.__class__ or whitelist == self._whitelist:
            return False
        self._whitelist = whitelist
        del_addrs = []
